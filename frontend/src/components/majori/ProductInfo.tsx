...
interface ProductInfoProps {
  ...
  onAddToCart?: (quantity: number, selectedSize?: string | null) => void;
  onBuyNow?: (quantity: number, selectedSize?: string | null) => void;
}

const ProductInfo = ({ data, onAddToCart, onBuyNow }: ProductInfoProps) => {
  ...
  return (
    <div>
      <p className="text-xs uppercase tracking-widest text-brand mb-2">{data.brand}</p>
      <h1 className="font-display text-3xl md:text-4xl font-semibold mb-3">{data.name}</h1>
      ...
      <div className="flex items-baseline gap-3 mb-6">
        ...
        {data.oldPrice && <span className="text-mute line-through">{data.oldPrice}</span>}
        {data.savePct && (
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <button
          type="button"
          onClick={() => onAddToCart?.(qty, size)}
          className="flex-1 min-w-[200px] bg-ink text-white px-7 h-12 uppercase tracking-wider text-sm hover:bg-brand transition rounded"
        >
          Add to Cart
        </button>
        <button
      ...
      <button
        type="button"
        onClick={() => onBuyNow?.(qty, size)}
        className="block w-full text-center bg-brand text-white px-7 py-3 uppercase tracking-wider text-sm hover:bg-ink transition rounded mb-8"
      >
        Buy It Now
      </button>
