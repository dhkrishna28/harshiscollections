...
const iconClassName = "inline-flex h-16 w-16 items-center justify-center rounded-xl bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-white/90";
const badgeClassName = "inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-sm font-medium bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500";
...
function StatCard({ icon, value, label, badge }: StatCardProps) {
  return (
...
export default function OrderStats() {
  ...
  const revenueGrowth =
    data && data.last_month_revenue > 0
      ? (
          ((data.month_revenue - data.last_month_revenue) /
  ...
  const stats = [
    {
      ...
      value: isLoading ? "—" : String(data?.total_orders ?? 0),
      label: "Total Orders",
      badge: isLoading ? "..." : `${data?.pending_orders ?? 0} pending`,
    },
    {
      icon: <InTransitIcon />,
    ...
    },
    {
      ...
      value: isLoading
        ? "—"
        : `₹${(data?.month_revenue ?? 0).toLocaleString("en-IN")}`,
